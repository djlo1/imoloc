...
const ROLES = [
  ...
  { id:'app_admin', label:'Administrateur des applications', desc:'Gère les applications et permissions.', badge:'Standard', badgeColor:'#0078d4' },
  { id:'role_admin', label:'Administrateur des rôles', desc:'Gère l\'attribution des rôles. Très sensible.', badge:'Critique', badgeColor:'#ef4444' },
  { id:'security_reader', label:'Lecteur de sécurité', desc:'Lecture seule des infos de sécurité.', badge:'Limité', badgeColor:'#6c63ff' },
...
export default function AddUserModal({ onClose, agenceName='Mon organisation', agenceId=null }) {
  ...
  const [form, setForm] = useState({
    ...
    username:'', email:'', agence: agenceName,
  ...
  const canNext = () => {
    if (step===1) return form.prenom.trim() && form.nom.trim() && form.email.trim()
    return true
  }
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      const password = form.auto_password
      ...
      // Fallback : créer via signUp classique
      ...
      if (userId) {
        ...
        // 3. Associer à l'agence si fourni
        if (agenceId) {
      ...
      // Si l'API admin n'est pas dispo, on sauvegarde quand même localement
  ...
  return (
    <>
      <style>{`
        ...
        .au-steps{width:220px;border-right:1px solid rgba(255,255,255,0.07);padding:24px 0;flex-shrink:0;display:flex;flex-direction:column}
        .au-step{display:flex;align-items:center;gap:14px;padding:11px 24px;cursor:pointer;position:relative}
        ...
        .au-step-lbl{font-size:13px;color:rgba(255,255,255,0.4);font-weight:400}
        .au-step-lbl.active{color:#e6edf3;font-weight:600}
        .au-step-lbl.done{color:rgba(255,255,255,0.55)}
        .au-step-line{position:absolute;left:36px;top:37px;width:2px;height:calc(100% - 11px);background:rgba(255,255,255,0.07)}
            ...
            <div className="au-content">
              ...
              {step===1 && (
                <>
                  ...
                  <div className="au-content-sub">Renseignez les informations de base sur la personne que vous ajoutez.</div>
                  <div className="au-grid2">
                    ...
                    <div className="au-field au-field-full">
                      <label className="au-lbl">Email professionnel <span>*</span></label>
                      <input type="email" className="au-input" value={form.email} onChange={e=>set('email',e.target.value)} placeholder="jean.dupont@organisation.com"/>
                    </div>
                    <div className="au-field au-field-full">
                      <label className="au-lbl">Nom d'utilisateur</label>
                      <div className="au-username-wrap">
                        <input className="au-input" style={{flex:1}} value={form.username} onChange={e=>set('username',e.target.value)} placeholder={form.prenom?form.prenom.toLowerCase():'jean.dupont'}/>
                  ...
                  <div style={{height:'1px',background:'rgba(255,255,255,0.07)',margin:'4px 0 18px'}}/>
                  <div className="au-checkbox-wrap" onClick={()=>set('auto_password',!form.auto_password)}>
                    ...
                    <div><div className="au-checkbox-lbl">Créer automatiquement un mot de passe</div><div className="au-checkbox-sub">Un mot de passe sécurisé sera généré automatiquement</div></div>
                  </div>
                  {!form.auto_password && <input type="password" className="au-input" value={form.password} onChange={e=>set('password',e.target.value)} placeholder="Mot de passe sécurisé..." style={{marginBottom:10}}/>}
              ...
              {step===3 && (
                <>
                  ...
                  <div className="au-info-box">
                    <svg width="14" height="14" fill="none" stroke="#4da6ff" strokeWidth="1.5" viewBox="0 0 24 24"><path strokeLinecap="round" d="M11.25 11.25l.041-.02a.75.75 0 011.063.852l-.708 2.836a.75.75 0 001.063.853l.041-.021M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-9-3.75h.008v.008H12V8.25z"/></svg>
                    L'accès dépend aussi des licences attribuées et du plan de votre organisation.
                  </div>
              ...
              {step===4 && (
                <>
                  ...
                  <div style={{maxHeight:240,overflowY:'auto',marginBottom:4,paddingRight:4}}>
                    {ROLES.map(role => (
                  ...
                  <div className="au-grid2">
                    ...
                    <div className="au-field"><label className="au-lbl">Équipe / Groupe</label><input className="au-input" value={form.groupes} onChange={e=>set('groupes',e.target.value)} placeholder="Agence Cotonou..."/></div>
                  </div>
                  ...
                  <div className="au-grid2">
                    ...
                    <div className="au-field"><label className="au-lbl">Email secondaire</label><input type="email" className="au-input" value={form.email_secondaire} onChange={e=>set('email_secondaire',e.target.value)} placeholder="jean@gmail.com"/></div>
                ...
                <>
                  ...
                  <div className="au-summary">
                    ...
                    {[
                      ...
                      {k:'Nom d\'utilisateur', v:`${form.username||form.prenom.toLowerCase()}@${form.agence}`},
                  ...
                  <div className="au-info-box">
                    <svg width="14" height="14" fill="none" stroke="#4da6ff" strokeWidth="1.5" viewBox="0 0 24 24"><path strokeLinecap="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75"/></svg>
                    Un email d'invitation sera envoyé à <strong>{form.email}</strong> avec ses identifiants.
                  </div>
                </>
              )}
            </div>
            ...
            <div className="au-foot-btns">
              ...
              {step<STEPS.length ? (
                ...
                <button className="au-btn au-btn-blue" disabled={saving} onClick={handleFinish}>
                  {saving?<><span className="au-spin"/>Création…</>:'✅ Terminer'}
                </button>
              )}
            </div>
          </div>
...
MODALEOFcat > src/pages/agence/components/AddUserModal.jsx << 'MODALEOF'
import { useState } from 'react'
import { supabase } from '../../../lib/supabase'
import toast from 'react-hot-toast'
