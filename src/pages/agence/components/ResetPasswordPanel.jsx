...
export default function ResetPasswordPanel({ onClose, agenceId }) {
  const [step, setStep] = useState(1) // 1=select, 2=options, 3=result
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      const { data } = await supabase.from('profiles').select('id, email, nom, prenom, role')
      setUsers(data || [])
    } finally {
      setLoading(false)
    }
  ...
  const handleReset = async () => {
    ...
    for (const user of selected) {
      ...
      try {
        // Mettre à jour via Supabase Auth Admin (Edge Function)
        const SUPABASE_URL = 'https://zecyfnurrcslukxvmpca.supabase.co'
        
        // Stocker le nouveau mot de passe dans invitations pour référence
  ...
  const getRoleColor = (role) => {
    const colors = {
      global_admin: '#ef4444', user_admin: '#f59e0b',
      agent: '#0078d4', comptable: '#6c63ff', lecteur: '#6c63ff',
    }
    return colors[role] || '#0078d4'
  }
  ...
  return (
    <>
      <style>{`
        ...
        .rp-panel{width:420px;height:100%;background:#161b22;display:flex;flex-direction:column;animation:rp-slide 0.2s ease;border-left:1px solid rgba(255,255,255,0.07)}
        @keyframes rp-slide{from{transform:translateX(100%)}to{transform:translateX(0)}}
        .rp-head{display:flex;align-items:center;justify-content:space-between;padding:18px 22px;border-bottom:1px solid rgba(255,255,255,0.07);flex-shrink:0}
        ...
        .rp-title{font-size:18px;font-weight:700;color:#e6edf3;padding:18px 22px 8px;flex-shrink:0}
        .rp-sub{font-size:13px;color:rgba(255,255,255,0.4);padding:0 22px 16px;line-height:1.6;flex-shrink:0}
        .rp-body{flex:1;overflow-y:auto;padding:0 22px}
        ...
        .rp-select-all-lbl{font-size:13px;font-weight:600;color:rgba(255,255,255,0.6)}
        ...
        .rp-user-item{display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid rgba(255,255,255,0.04);cursor:pointer;transition:background 0.1s}
        .rp-user-item:hover{background:rgba(255,255,255,0.02);border-radius:6px;padding:10px 6px;margin:0 -6px}
        ...
        .rp-role-badge{font-size:10px;font-weight:600;padding:1px 7px;border-radius:100px;margin-left:6px}
        ...
        .rp-selected-list{margin-bottom:20px}
        .rp-selected-chip{display:inline-flex;align-items:center;gap:6px;padding:4px 10px;border-radius:100px;background:rgba(0,120,212,0.12);border:1px solid rgba(0,120,212,0.25);font-size:12px;color:#4da6ff;margin:0 4px 6px 0}
        ...
        .rp-result-pass-val{font-family:monospace;font-size:14px;color:#00c896;font-weight:600;letter-spacing:0.05em}
        ...
        .rp-success-icon{width:20px;height:20px;border-radius:50%;background:rgba(0,200,150,0.15);border:1.5px solid rgba(0,200,150,0.3);display:flex;align-items:center;justify-content:center;flex-shrink:0}
        .rp-foot{padding:16px 22px;border-top:1px solid rgba(255,255,255,0.07);display:flex;gap:10px;flex-shrink:0;background:#161b22}
        ...
        .rp-spin{width:14px;height:14px;border:2px solid rgba(255,255,255,0.2);border-top-color:#fff;border-radius:50%;animation:rp-s 0.6s linear infinite;display:inline-block}
        ...
        <div className="rp-panel">
          ...
          {step === 1 && (
            <>
              ...
              <div className="rp-sub">
                Sélectionnez les utilisateurs dont vous souhaitez réinitialiser le mot de passe.
              </div>
              <div className="rp-body">
                ...
                {filtered.length > 0 && (
                  <div className="rp-select-all" onClick={toggleAll}>
                    ...
                    <span className="rp-select-all-lbl">Sélectionner tout</span>
                  </div>
                )}
                  ...
                  filtered.map((u,i) => {
                    ...
                    return (
                      <div key={i} className="rp-user-item" onClick={()=>toggleSelect(u)}>
                        ...
                        <div className="rp-avatar" style={{background:`linear-gradient(135deg,${color},${color}99)`}}>
                          {getInitials(u)}
                        </div>
                        <div style={{flex:1,minWidth:0}}>
                          <div className="rp-user-name">
                            {u.prenom} {u.nom}
                            {u.role && (
                              <span className="rp-role-badge" style={{background:`${color}18`,color}}>
                                {u.role}
                              </span>
                            )}
                          </div>
                          <div className="rp-user-email">{u.email}</div>
                        </div>
                      </div>
                    )
                  })
          ...
          {step === 2 && (
            <>
              ...
              <div className="rp-body">
                {/* Chips utilisateurs sélectionnés */}
                <div className="rp-selected-list">
                  {selected.map((u,i) => (
                    <span key={i} className="rp-selected-chip">
                      {u.prenom} {u.nom}
                    </span>
                  ))}
                </div>
                ...
                <div
                  className={`rp-option ${autoPass?'active':''}`}
                  ...
                  <div>
                    ...
                    <div className="rp-option-sub">Un mot de passe sécurisé sera généré pour chaque utilisateur</div>
                  </div>
                </div>
                ...
                <div
                  className={`rp-option ${forceChange?'active':''}`}
                  ...
                  <div>
                    <div className="rp-option-lbl">Demander à ces utilisateurs de modifier leur mot de passe lors de sa première connexion</div>
                  </div>
                </div>
              </div>
          ...
          {step === 3 && (
            <>
              <div className="rp-title" style={{display:'flex',alignItems:'center',gap:10}}>
                <div style={{width:28,height:28,borderRadius:'50%',background:'rgba(0,200,150,0.15)',border:'2px solid rgba(0,200,150,0.3)',display:'flex',alignItems:'center',justifyContent:'center'}}>
                ...
                {results.map((r,i) => (
                  <div key={i} className="rp-result-item">
                    <div className="rp-result-user">
                      ...
                      <div>
                        <div style={{fontSize:13.5,fontWeight:600,color:'#e6edf3'}}>{r.user.prenom} {r.user.nom}</div>
                        <div style={{fontSize:12,color:'rgba(255,255,255,0.35)'}}>{r.user.email}</div>
                      </div>
                    </div>
                    <div style={{fontSize:11,fontWeight:600,color:'rgba(255,255,255,0.3)',textTransform:'uppercase',letterSpacing:'0.07em',marginBottom:6}}>Nouveau mot de passe</div>
                    <div className="rp-result-pass">
                      <span className="rp-result-pass-val">{r.password}</span>
                    ...
                    {forceChange && (
                      <div style={{fontSize:12,color:'rgba(245,158,11,0.7)',marginTop:8}}>
                        ⚠️ L'utilisateur devra modifier ce mot de passe à sa prochaine connexion
                      </div>
                    )}
                  </div>
                ))}
